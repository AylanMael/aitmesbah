...
import { createStagingPolicy } from "./staging-policy.mjs";
...
export function createServerConfig(env,{requireHmac=false,activateRemote=false}={}) {
  ...
  if(appEnv==="staging")return createStagingPolicy(env,{activateRemote});
  if(appEnv!=="local") {
    ...
    if(canonical.protocol!=="https:"||canonical.username||canonical.password||canonical.hash||canonical.origin.includes("localhost")||canonical.origin.includes("127.0.0.1")||canonical.hostname.startsWith("staging.")) configurationError("REMOTE_URL_INVALID");
    const projectId=env.NEXT_PUBLIC_FIREBASE_PROJECT_ID;
    if(!projectId||projectId.startsWith("demo-")||(env.AITMESBAH_STAGING_PROJECT_ID&&projectId===env.AITMESBAH_STAGING_PROJECT_ID)||env.GCLOUD_PROJECT!==projectId)configurationError("REMOTE_PROJECT_CONFLICT");
    ...
    const activated=env.AITMESBAH_REMOTE_ACTIVATION==="enabled"&&env.NEXT_PUBLIC_AITMESBAH_REMOTE_ACTIVATION==="enabled";
    if(activateRemote&&!activated) configurationError("REMOTE_INITIALIZATION_BLOCKED");
    return Object.freeze({appEnv,projectId,storageBucket:env.NEXT_PUBLIC_FIREBASE_STORAGE_BUCKET,canonicalUrl:canonical.origin,hosts,origins,emulated:false,remoteInitializationAllowed:activated,cookie:cookiePolicyFor(appEnv),csrfCookie:Object.freeze({name:`__Host-aitmesbah_csrf${appEnv==="staging"?"_staging":""}`,httpOnly:true,secure:true,sameSite:"strict",path:"/",maxAge:600}),appCheck:appEnv==="staging"?"observe":env.AITMESBAH_APP_CHECK,rateLimit:Object.freeze({distributedRequired:true,configured:env.AITMESBAH_RATE_LIMIT_BACKEND==="firestore",productive:env.AITMESBAH_RATE_LIMIT_BACKEND==="firestore"})});
  }
  ...
  return Object.freeze({appEnv,projectId,storageBucket:`${projectId}.appspot.com`,emulators,emulated:true,hosts:LOCAL_HOSTS,origins:LOCAL_ORIGINS,cookie:cookiePolicyFor(appEnv),csrfCookie:Object.freeze({name:"aitmesbah_csrf_local",httpOnly:true,secure:false,sameSite:"strict",path:"/",maxAge:600}),appCheck:"disabled-local",rateLimit:Object.freeze({distributedRequired:false,configured:false,productive:false})});
}
