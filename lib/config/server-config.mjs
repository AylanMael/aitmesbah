...
export function createServerConfig(env,{requireHmac=false,activateRemote=false}={}) {
  ...
  if(appEnv!=="local") {
    ...
    if(env.NEXT_PUBLIC_FIREBASE_PROJECT_ID?.startsWith("demo-")||(env.NEXT_PUBLIC_FIREBASE_PROJECT_ID&&env.AITMESBAH_STAGING_PROJECT_ID&&env.NEXT_PUBLIC_FIREBASE_PROJECT_ID===env.AITMESBAH_STAGING_PROJECT_ID))configurationError("REMOTE_PROJECT_CONFLICT");
