...
export function createPublicConfig(env) {
  ...
  if(env.NEXT_PUBLIC_AITMESBAH_REMOTE_ACTIVATION!=="enabled")configurationError("REMOTE_INITIALIZATION_BLOCKED");
  return Object.freeze(assertPublicShape({appEnv,projectId,apiKey:required(env.NEXT_PUBLIC_FIREBASE_API_KEY,"PUBLIC_CONFIG_INCOMPLETE"),authDomain:required(env.NEXT_PUBLIC_FIREBASE_AUTH_DOMAIN,"PUBLIC_CONFIG_INCOMPLETE"),appId:required(env.NEXT_PUBLIC_FIREBASE_APP_ID,"PUBLIC_CONFIG_INCOMPLETE"),storageBucket:required(env.NEXT_PUBLIC_FIREBASE_STORAGE_BUCKET,"PUBLIC_CONFIG_INCOMPLETE"),useEmulators:false,authEmulatorUrl:null,appCheck:env.NEXT_PUBLIC_AITMESBAH_APP_CHECK_MODE??"observe"}));
}
