...
export const AUDIT_CATEGORIES = Object.freeze(["account","authentication","session","invitation","organization","membership","authorization","contribution","editorial","asset","rights","consent","security"]);
export const AUDIT_RESULTS = Object.freeze(["success","denied","failed"]);
export const AUDIT_EVENT_CATALOG = Object.freeze({
  "account.invited":"account", "account.status_changed":"account", "account.roles_changed":"account",
  "invitation.created":"invitation", "invitation.consumed":"invitation", "invitation.revoked":"invitation", "invitation.expired":"invitation",
  "authentication.login_succeeded":"authentication", "authentication.login_failed":"authentication", "authentication.email_verified":"authentication", "authentication.recovery_completed":"authentication",
  "session.created":"session", "session.logout_local":"session", "session.revoked":"session", "session.revoked_all":"session",
...
export function normalizeLegacyAudit(legacy,{eventId,correlationId=eventId,actorType="local_admin",result="success"}={}){
  ...
  const targetType=eventType.startsWith("account.")?"account":eventType.startsWith("authentication.")?"account":eventType.startsWith("session.")?"session":eventType.startsWith("invitation.")?"invitation":eventType.startsWith("organization.")?"organization":eventType.startsWith("membership.")?"membership":eventType.startsWith("asset.")?"asset":eventType.startsWith("rights.")?"rights":eventType.startsWith("consent.")?"consent":"contribution";
  ...
  return createAuditEvent({eventId,eventType,category:AUDIT_EVENT_CATALOG[eventType],actorUid:legacy.actorUid??null,actorType,targetType,targetId,organizationId:legacy.organizationId??null,contributionId:legacy.contributionId??null,assetId:legacy.assetId??null,previousState,nextState,changedFields:legacy.changedFields??(previousState===nextState?[]:[targetType==="account"?"status":"state"]),reason:legacy.reason??null,result,occurredAt:legacy.occurredAt,correlationId,schemaVersion:1});
}
