...
export const RATE_LIMIT_COUNTER_COLLECTION = "rateLimitCounters";
export const MAX_RATE_LIMIT_COUNTERS = 6;
export const MAX_ROTATION_READS = 12;
...
export const STAGING_RATE_LIMIT_PROFILES = Object.freeze({
  ...
  contentMutation: profile("contentMutation", [
    { identity: "uid", windows: [{ seconds: 60, maximum: 20 }] },
...
function pseudonym(secret, kind, raw) {
  return createHmac("sha256", secret).update(RATE_LIMIT_POLICY_VERSION).update("\0").update(kind).update("\0").update(raw).digest("base64url");
}

function counterDocumentId({ category, generationId, identity, pseudonymousValue, windowSeconds, windowStartMs }) {
  return createHash("sha256")
    .update("rate-limit-document-v1\0")
...
export function buildRateLimitPlan({ profileName, identities, generations, nowMs, cost = 1 }) {
  ...
  for (const counter of selected.counters) {
    ...
    for (const window of counter.windows) {
      ...
      const documents = generationValues.map(generation => {
        const pseudonymousValue = pseudonym(generation.secret, counter.identity, raw);
        return Object.freeze({
          documentId: counterDocumentId({ category: selected.category, generationId: generation.id, identity: counter.identity, pseudonymousValue, windowSeconds: window.seconds, windowStartMs }),
  ...
  const reads = logicalCounters.reduce((total, counter) => total + counter.documents.length, 0);
  if (reads > MAX_ROTATION_READS) throw closedError("maximum de lectures dépassé");
  return Object.freeze({
