...
import {canonicalFingerprint,signCursor,verifyCursor} from "./cursor-signing.mjs";
...
function fingerprint(filters){return canonicalFingerprint(FILTERS.map(key=>[key,filters[key]??null]));}
...
export function encodeAuditCursor(position,filters={}){assert.match(position.eventId,IDS);assert.ok(Number.isFinite(position.occurredAtMillis));return signCursor({e:position.eventId,t:position.occurredAtMillis,f:fingerprint(filters)},{purpose:"crm-audit-v1"});}
export function decodeAuditCursor(cursor,filters={}){try{const payload=verifyCursor(cursor,{purpose:"crm-audit-v1"});assert.deepEqual(Object.keys(payload).sort(),["e","f","t","v"]);assert.match(payload.e,IDS);assert.ok(Number.isFinite(payload.t));assert.equal(payload.f,fingerprint(filters),"curseur incompatible avec les filtres");return Object.freeze({eventId:payload.e,occurredAtMillis:payload.t});}catch(error){if(error?.message?.includes("incompatible"))throw error;throw new TypeError("curseur invalide");}}
