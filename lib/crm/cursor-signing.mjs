...
import { loadLocalHmacSecret } from "../config/server-config.mjs";

const MAX_CURSOR_LENGTH = 768;
function secret(value) {
  try { return value === undefined ? loadLocalHmacSecret(process.env) : loadLocalHmacSecret({CRM_CURSOR_HMAC_SECRET:value}); }
  catch { throw new Error("secret de curseur indisponible"); }
}
