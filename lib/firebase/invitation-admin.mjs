...
function activationRecordToFirestore(record){validateActivationRecord(record);const raw={...record};for(const field of ACTIVATION_TIMESTAMP_FIELDS)raw[field]=record[field]===null?null:Timestamp.fromMillis(record[field]);validateExactRawActivation(raw);return raw;}
