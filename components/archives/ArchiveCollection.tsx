...
export default function ArchiveCollection({ publications }: { publications: ArchiveCard[] }) {
  ...
  return <div className="archive-collection">
    <div className="archive-collection-search"><label htmlFor="archive-search">Chercher dans la collection</label><input id="archive-search" type="search" value={query} onChange={event => setQuery(event.target.value)} placeholder="Un lieu, un métier, une année…" aria-controls="archive-results" /></div>
    <div className="archive-collection-filters" role="group" aria-label="Types de documents">{categories.map(item => <button key={item} type="button" aria-pressed={category === item} aria-controls="archive-results" onClick={() => setCategory(item)}>{item}<span>{item === "Tout" ? entries.length : entries.filter(entry => entry.type === item).length}</span></button>)}</div>
    ...
    {!matches.length && <div className="archive-collection-empty"><h3>Aucun document ne correspond pour le moment.</h3><p>Essayez un autre mot ou explorez l’ensemble de la collection.</p><button type="button" onClick={() => { setQuery(""); setCategory("Tout"); }}>Réinitialiser la recherche</button></div>}
  </div>;
}
