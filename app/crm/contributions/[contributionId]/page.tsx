...
export default async function ContributionRecordPage({params}:{params:Promise<{contributionId:string}>}){
 ...
 return <main className="crm-shell crm-record-page">
  ...
  <div className="crm-record-layout"><section className="crm-record-document"><header><div><p className="crm-kicker">Version de travail</p><h2>Le contenu du dossier</h2></div><span>Version {contribution.currentVersion}</span></header><article className="structured-text"><StructuredText value={String(body)}/></article></section><aside className="crm-record-facts"><p className="crm-kicker">Fiche documentaire</p><h2>Contexte et repères</h2><dl><div><dt>Date ou période</dt><dd>{meta.archiveDate||"À préciser"}</dd></div><div><dt>Auteur ou producteur</dt><dd>{meta.creator||"À préciser"}</dd></div><div><dt>Lieu</dt><dd>{meta.location||"À préciser"}</dd></div><div><dt>Provenance</dt><dd>{meta.provenance||"À préciser"}</dd></div><div><dt>Crédit et droits</dt><dd>{meta.rightsCredit||"À préciser"}</dd></div><div><dt>Mots-clés</dt><dd>{meta.tags.length?meta.tags.join(" · "):"À préciser"}</dd></div></dl><a href="#fichiers-du-dossier">Voir les fichiers du dossier ↓</a></aside></div>
  <section id="fichiers-du-dossier" className="crm-contributions" style={{scrollMarginTop:100}} aria-labelledby="dossier-fichiers-titre"><h2 id="dossier-fichiers-titre">Les fichiers du dossier</h2><p>Les originaux restent privés. Leur ajout ne les valide pas et ne les publie pas.</p>{session.permissions.some(p=>["asset.self.manage","asset.assigned.read","asset.assigned.review","asset.deletion.manage"].includes(p))?<AssetManager contributions={[]} embeddedContribution={{contributionId:contribution.contributionId,title:contribution.title}} permissions={session.permissions.filter(p=>p!=="asset.self.manage"||contribution.authorUid===session.uid)}/>:<p>Votre rôle ne permet pas de consulter les fichiers de ce dossier.</p>}</section>
