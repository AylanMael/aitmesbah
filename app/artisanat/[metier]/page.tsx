...
export default async function MetierPage({params}:{params:Promise<{metier:string}>}){
  ...
  <nav className="craft-chapters" aria-label="Dans cette page">{craft.sections.map(([title],index)=><a href={`#chapitre-${index+1}`} key={title}><span>0{index+1}</span>{title}<span aria-hidden="true">↓</span></a>)}</nav>
