...
export async function GET(request:NextRequest){try{validateCrmRead(request);const actor=await requireCrmActor("profile.assigned.read");return NextResponse.json(await listAccounts(actor,Object.fromEntries(request.nextUrl.searchParams)),{headers:CRM_HEADERS});}catch(error){return crmError(error);}}
