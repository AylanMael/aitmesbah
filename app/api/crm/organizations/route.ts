...
export async function GET(request:NextRequest){try{const actor=await actorAny(["organization.list","organization.member.manage","role.local.manage"]);return NextResponse.json(await listOrganizations(actor.uid,Object.fromEntries(request.nextUrl.searchParams)),{headers:CRM_HEADERS});}catch(error){return crmError(error);}}
